...
Most notable changes include:
  ...
  * _Looking for suggestions to go here_
  * Android platform includes a new whitelist for controlling the launch of external applications via intents. Also includes support for gradle.
...
While preparing the 3.6.0 release, a release candidate was published to a write-once repository. When it was discovered that the release candidate needed to be updated, we were unable to modify the write-once repository, so we were forced to bump the version number. This is why all the 3.6 candence release items are labelled as "3.6.1". So 3.6.1 is the first release of the 3.6.x version. Apologies for the confusion.
...
* [CB-4655](https://issues.apache.org/jira/browse/CB-4655) WP8: Default native project template should be overridable at project creation time
...
* Removed Windows7 which is now in it's own branch.
* Added list of supported architectures in help text
* Adds support for target architectures to build command
